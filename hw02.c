...
const char* text[] = {"None","move and discard","weird square","texture - circle","noise","snow ish"};
...
{
   //  Initialize GLFW
   GLFWwindow* window = InitWindow("HW02 Kelley Kelley",1,600,600,&reshape,&key);

   //  Load shaders
   shader[1] = CreateShaderProg("randoms.vert","randoms.frag");
   shader[2] = CreateShaderProg("plbt.vert","plbt.frag");
   shader[3] = CreateShaderProg("plbt.vert","circle.frag");
   shader[4] = CreateShaderProg("plbt.vert","noise.frag");
   shader[5] = CreateShaderProg("plbt.vert","snow.frag");
   //  Load textures
